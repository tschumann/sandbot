...
extern float fTeamplay;

void BotFixIdealPitch( edict_t *pEdict )
{
    // check for wrap around of angle...
    if( pEdict->v.idealpitch >= 180.0 )
	{
        pEdict->v.idealpitch -= 360.0 * ((int)(pEdict->v.idealpitch / 360.0) + 1.0);
	}
...
{
    // check for wrap around of angle...
    if( pEdict->v.ideal_yaw >= 180.0 )
	{
        pEdict->v.ideal_yaw -= 360.0 * ((int)(pEdict->v.ideal_yaw / 360.0) + 1.0);
	}
...
{
    // check for wrap around of angle
    if( pEdict->v.angles.x >= 180.0 )
	{
        pEdict->v.angles.x -= 360.0 * ((int)(pEdict->v.angles.x / 360.0) + 1.0);
	}
    ...
    if( pEdict->v.angles.y >= 180.0 )
	{
        pEdict->v.angles.y -= 360.0 * ((int)(pEdict->v.angles.y / 360.0) + 1.0);
	}
    ...
    if( pEdict->v.angles.z >= 180.0 )
	{
        pEdict->v.angles.z -= 360.0 * ((int)(pEdict->v.angles.z / 360.0) + 1.0);
	}

    if( pEdict->v.angles.z < -180.0 )
	{
        pEdict->v.angles.z += 360.0 * ((int) (-pEdict->v.angles.z / 360) + 1);
	}
}
...
void BotFixViewAngles( edict_t *pEdict )
{
	...
	{
        pEdict->v.v_angle.x -= 360.0 * ((int)(pEdict->v.v_angle.x / 360.0) + 1.0);
	}
	...
	{
        pEdict->v.v_angle.y -= 360.0 * ((int)(pEdict->v.v_angle.y / 360.0) + 1.0);
	}
    ...
    if( pEdict->v.v_angle.z >= 180.0 )
	{
        pEdict->v.v_angle.z -= 360.0 * ((int)(pEdict->v.v_angle.z / 360.0) + 1.0);
	}
...
bool BotFindWaypoint( bot_t *pBot )
{
	...
	pBot->curr_waypoint_index = WaypointFindNext( pBot, 500.0, UTIL_GetTeam( pBot->pEdict ) );
...
bool BotHeadTowardWaypoint( bot_t *pBot )
{
	...
	// check if nearest waypoint is still the current waypoint
	if( WaypointFindNearest( pBot, 500.0, UTIL_GetTeam( pBot->pEdict ) ) == pBot->curr_waypoint_index )
	{
		return false;
	}

	// attempt to find a nearby waypoint
    if( BotFindWaypoint( pBot ) )
	{
		// let's head off toward that waypoint
		pBot->waypoint_origin = waypoints[pBot->curr_waypoint_index].origin;
		Vector v_waypoint = waypoints[pBot->curr_waypoint_index].origin - pBot->pEdict->v.origin;
        Vector v_botangles = UTIL_VecToAngles( v_waypoint );

        pBot->pEdict->v.ideal_yaw = v_botangles.y;
...
bool BotFindWaypoint( bot_t *pBot )
{
	...
	float fMinimumDistances[3];

	int index, select_index, path_index;
	
	int min_index[3];
	
	for( int i = 0; i < 3; i++ )
	{
		fMinimumDistances[i] = 9999.0;
		min_index[i] = -1;
	}
	
	index = WaypointFindPath( &pPath, &path_index, pBot->curr_waypoint_index, iTeam );
	...
	while( index != -1 )
	{
			...
			(index != pBot->prev_waypoint_index[4]))
		{
			...
			float fDistance = (pEdict->v.origin - waypoints[index].origin).Length();
			
			if( fDistance < fMinimumDistances[0] )
			{
				fMinimumDistances[2] = fMinimumDistances[1];
				min_index[2] = min_index[1];
				
				fMinimumDistances[1] = fMinimumDistances[0];
				min_index[1] = min_index[0];
				
				fMinimumDistances[0] = fDistance;
				min_index[0] = index;
			}
			else if( fDistance < fMinimumDistances[1] )
			{
				fMinimumDistances[2] = fMinimumDistances[1];
				min_index[2] = min_index[1];
				
				fMinimumDistances[1] = fDistance;
				min_index[1] = index;
			}
			else if( fDistance < fMinimumDistances[2] )
			{
				fMinimumDistances[2] = fDistance;
				min_index[2] = index;
			}
		}
...
bool BotHeadTowardWaypoint( bot_t *pBot )
{
	...
	if( ( pBot->waypoint_goal == -1 ) && ( pBot->f_waypoint_goal_time > gpGlobals->time + 2.0 ) && pBot->f_waypoint_goal_time )
	{
	...
	{		
		...
		if( ( pBot->f_end_use_ladder_time + 2.0 ) > gpGlobals->time || ( pBot->pEdict->v.waterlevel == 3 ) )
		{
			...
			i = WaypointFindNearest( pEdict, REACHABLE_RANGE, pEdict->v.team );
		}
		else
		{
			// find the nearest reachable waypoint
			i = WaypointFindReachable( pEdict, REACHABLE_RANGE, pEdict->v.team );
		}
	...
	{
		// don't pick a goal more often than every 120 seconds...
		pBot->f_waypoint_goal_time = gpGlobals->time + 120.0;
			
		if( pEdict->v.team == TEAM_ALIEN )
		{
			index = WaypointFindNearestGoal( pBot->pEdict, pBot->curr_waypoint_index, pEdict->v.team, W_FL_HIVE );
		}
		else
		{
			index = WaypointFindNearestGoal( pBot->pEdict, pBot->curr_waypoint_index, pEdict->v.team, W_FL_CONSOLE );
		}
	...
	if( ( pBot->prev_waypoint_distance > 1.0 ) && ( waypoint_distance > pBot->prev_waypoint_distance ) && ( waypoint_distance < min_distance ) )
	{
		touching = true;
	}
	...
	if( touching )
	{
		...
		{
			// a little delay time, since we'll touch the waypoint before we actually get what it has
		...
		if (pBot->pEdict->v.waterlevel == 3)
		{
			...
			if (tr.flFraction >= 1.0)
			{
				...
				{
					// find the nearest visible waypoint
					i = WaypointFindNearest(tr.vecEndPos, pEdict, 100, pEdict->v.team);
		...
		{	// are we trying to get an item?
			int nextwaypoint = pBot->waypoint_goal;
		
			
			i = WaypointRouteFromTo(pBot->curr_waypoint_index, nextwaypoint, pEdict->v.team);
	...
	if( !pBot->pBotEnemy )
	{
		if( !pBot->pBotPickupItem || (pBot->pBotPickupItem && !(FStrEq(STRING(pBot->pBotPickupItem->v.classname), "func_healthcharger") ||
...
bool BotHeadTowardWaypoint( bot_t *pBot )
{
   ...
   else if (( g_iMod == GEARBOX_DLL) && (pent_info_ctfdetect != NULL))
   {
      pent = NULL;
   ...
   if (touching)
   {
      ...
      {
         if (((g_iMod == TFC_DLL) && (pEdict->v.playerclass == TFC_CLASS_SNIPER)) ||
             (g_iMod != TFC_DLL))
         {
            int aim_index;
      ...
      if (pBot->curr_waypoint_index == pBot->waypoint_goal)
      {
         ...
         if ((waypoints[pBot->curr_waypoint_index].flags & W_FL_TFC_SENTRYGUN) &&
             (g_iMod == TFC_DLL) && (pEdict->v.playerclass == TFC_CLASS_ENGINEER))
         {
...
void BotOnLadder( bot_t *pBot, float moved_distance )
{
    ...
    if (pBot->ladder_dir == LADDER_UNKNOWN)
    {
        ...
        while ((!done) && (angle < 180.0))
        {
            ...
            {
                if( !strcmp("func_wall", STRING(tr.pHit->v.classname)) )
                {
...
bool BotStuckInCorner( bot_t *pBot )
{
    ...
    v_src = pEdict->v.origin;
    v_dest = v_src + gpGlobals->v_forward * 20.0 + gpGlobals->v_right * 20.0;
...
bool BotFollowUser( bot_t *pBot )
{
	...
	{
        pEdict->v.v_angle.x = 0.0;  // reset pitch to 0 (level horizontally)
	}

    pEdict->v.v_angle.z = 0.0;  // reset roll to 0 (straight up and down)

    pEdict->v.angles.x = 0.0;
