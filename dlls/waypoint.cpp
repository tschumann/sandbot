...
// (-1 if not found)...
int WaypointFindNearest( bot_t *pBot, float range, int team )
{
    int i, min_index;
    float distance;
    float min_distance;
    ...
    for (i=0; i < num_waypoints; i++)
    {
		...
		// skip recent waypoints
		int j = 0;

		for( j = 0; j < PREVIOUS_WAYPOINTS; j++ )
		{
			if( pBot->prev_waypoint_index[j] == i )
			{
				break;
			}
		}
		...
		distance = (waypoints[i].origin - pBot->pEdict->v.origin).Length();

        if ((distance < min_distance) && (distance < range))
        {
            // if waypoint is visible from current position (even behind head)...
            UTIL_TraceLine( pBot->pEdict->v.origin + pBot->pEdict->v.view_ofs, waypoints[i].origin, ignore_monsters, pBot->pEdict->v.pContainingEntity, &tr );
...
int WaypointFindNearest( Vector v_src, bot_t *pBot, float range, int team )
{
    ...
    for (int i = 0; i < num_waypoints; i++)
    {
        ...
        if ((distance < min_distance) && (distance < range))
        {
            ...
            UTIL_TraceLine( v_src, waypoints[i].origin, ignore_monsters, pBot->pEdict->v.pContainingEntity, &tr );
...
int WaypointFindNearest( Vector v_src, edict_t *pEntity, float range, int team )
{
    ...
    for (int i = 0; i < num_waypoints; i++)
    {
        ...
        {
            // if waypoint is visible from source position...
            UTIL_TraceLine( v_src, waypoints[i].origin, ignore_monsters, pEntity->v.pContainingEntity, &tr );
...
void WaypointAdd( edict_t *pEntity )
{
    ...
    while( pent = UTIL_FindEntityInSphere( pent, pEntity->v.origin, radius ) )
    {
        strcpy(item_name, STRING(pent->v.classname));

        if( !strcmp( "item_healthkit", item_name) )
        {
            ClientPrint(pEntity, HUD_PRINTCONSOLE, "found a healthkit!\n");
            waypoints[index].flags = W_FL_HEALTH;
        }

        if( !strncmp( "item_armor", item_name, 10) )
        {
            ClientPrint(pEntity, HUD_PRINTCONSOLE, "found some armor!\n");
            waypoints[index].flags = W_FL_ARMOR;
        }

		if( !strcmp( "team_command", STRING(pent->v.classname) ) )
        {
            waypoints[index].flags = W_FL_NS_CONSOLE;
        }

		if( !strcmp( "team_hive", STRING(pent->v.classname) ) )
        {
...
bool WaypointLoad( edict_t *pEntity )
{
    ...
    if (bfp != NULL)
    {
        ...
        header.filetype[7] = 0;
        if ( !strcmp(header.filetype, "HPB_bot"))
        {
            ...
            if (strcmp(header.szMapname, STRING(gpGlobals->mapname)) == 0)
            {
...
void WaypointSave( void )
{
    ...
    strncpy(header.szMapname, STRING(gpGlobals->mapname), 31);
    header.szMapname[31] = 0;
    ...
    while (changed)
    {
        ...
        for (x=0; x < route_num_waypoints; x++)
        {
            for (y=0; y < route_num_waypoints; y++)
            {
                for (z=0; z < route_num_waypoints; z++)
                {
                    if ((shortest_path[y * route_num_waypoints + x] == WAYPOINT_UNREACHABLE) || (shortest_path[x * route_num_waypoints + z] == WAYPOINT_UNREACHABLE))
                        continue;
                    ...
                    if ((distance < shortest_path[y * route_num_waypoints + z]) || (shortest_path[y * route_num_waypoints + z] == WAYPOINT_UNREACHABLE))
                    {
...
int WaypointFindNext( bot_t *pBot, float range, int team )
{
	...
	for( int i = 0; i < num_waypoints; i++ )
	{
		...
		if( distance < min_distance )
		{
			...
			if( tr.flFraction >= 1.0 )
			{
				if( WaypointReachable( pBot->pEdict->v.origin, waypoints[i].origin, pBot->pEdict ) )
				{
					min_index = i;
					min_distance = distance;
				}
	...
	pBot->prev_waypoint_index[9] = pBot->prev_waypoint_index[8];
	pBot->prev_waypoint_index[8] = pBot->prev_waypoint_index[7];
	pBot->prev_waypoint_index[7] = pBot->prev_waypoint_index[6];
	pBot->prev_waypoint_index[6] = pBot->prev_waypoint_index[5];
	pBot->prev_waypoint_index[5] = pBot->prev_waypoint_index[4];
	...
	pBot->prev_waypoint_index[0] = min_index;
